...
- **Unit test for EVERY task**: With AI tools, create comprehensive unit tests for every single coding task
- **Tests as documentation**: Tests serve as living documentation of intended behavior
- **Test organization**: ALL tests MUST be in dedicated test folders (`tests/`, `__tests__/`, `test/`, `e2e/`)
- **Never co-locate tests**: NEVER place test files alongside source code in `src/` directory
- **Security scan exclusion**: Test folders must be excluded from security scanning tools (Snyk, SonarCloud, CodeQL, etc.)
...
### Test Organization and Security
...
- Configure `.snyk` to exclude test folders
- Configure `sonar-project.properties` to exclude test folders
- Configure `.github/codeql/codeql-config.yml` to exclude test folders
- Configure `.gitlab-ci.yml` SAST to exclude test folders
- Configure `.eslintrc.json` to ignore test patterns

**DO NOT add test folders to `.gitignore`** - tests must be version controlled

See detailed configuration: [Testing Guidelines](docs/testing-guidelines.md#test-organization-and-security-scanning)
...
- **Test Location**: Verify tests are in `tests/`, `__tests__/`, or `test/` folders
- **No src/ tests**: Ensure no test files exist in `src/` directory
- **Security exclusions**: Confirm security tools exclude test folders
